import type { Evaluation, EvaluationRunStatus, EvidenceItem, Incident, Report, Scenario, ServiceHealth } from "./types";
import type { IncidentPresentation } from "./incident-presentation";
...
export const api = {
  ...
  createIncident: (trafficBatchId?: string, presentation?: IncidentPresentation) => request<Incident>("/api/incidents", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...(trafficBatchId === undefined ? {} : { traffic_batch_id: trafficBatchId }), ...(presentation ?? {}) }) }),
  ...
  labHealth: () => request<ServiceHealth[]>("/api/lab/health"),
  evaluations: () => request<Evaluation[]>("/api/evaluations"),
  runEvaluation: () => request<EvaluationRunStatus>("/api/evaluations/run", { method: "POST" }),
  evaluationStatus: () => request<EvaluationRunStatus>("/api/evaluations/status"),
};
