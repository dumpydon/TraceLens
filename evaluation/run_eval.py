...
async def evaluate_example(example: dict[str, Any], traffic_count: int) -> dict[str, float]:
    clear_logs()
    activate_scenario(example["scenario"])
    batch_id = f"BATCH-EVAL-{uuid.uuid4().hex[:8].upper()}"
    started_at = datetime.now(timezone.utc)
    traffic = await generate_traffic(
        traffic_count, "http://127.0.0.1:8101", batch_id
    )
    ended_at = datetime.now(timezone.utc)
    if traffic.get("connection_error"):
        raise RuntimeError("Incident Lab services are not reachable; start both services before evaluation")
    ...
    report = database.get_report(incident.id)
    if not report:
    ...
    outputs = {
        ...
        "retrieved_failure_types": [item.failure_type for item in retrieved if item.failure_type],
        "citation_ids": [item.evidence_id for item in report.evidence],
        "available_evidence_ids": [item.id for item in evidence],
    }
    return {item["key"]: item["score"] for evaluator in EVALUATORS if (item := evaluator(outputs, example))}


async def run(traffic_count: int = 6) -> EvaluationSummary:
    Database().initialize()
    examples = json.loads((ROOT / "evaluation" / "dataset" / "incidents.json").read_text())
    scores = []
    ...
    summary = EvaluationSummary(
        ...
        examples=len(scores),
        root_cause_correctness=sum(item["root_cause_correctness"] for item in scores) / len(scores),
        affected_service_correctness=sum(item["affected_service_correctness"] for item in scores) / len(scores),
        retrieval_relevance=sum(item["retrieval_relevance"] for item in scores) / len(scores),
        evidence_groundedness=sum(item["evidence_groundedness"] for item in scores) / len(scores),
    )
    Database().save_evaluation(summary)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run the TraceLens offline evaluation dataset")
